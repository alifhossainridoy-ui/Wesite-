...
- **Purchase dedup architecture**: keep the three trigger points (`woocommerce_order_status_processing`, `woocommerce_order_status_completed`, `woocommerce_thankyou`) but formalize the guard: a persistent per-order meta flag (already-sent) checked before any send, one shared `event_id` reused across browser pixel and CAPI for the same order (for Meta-side dedup), and a **pre-send validity guard** that must pass before a Purchase event is sent at all: order status is `processing` or `completed`, `total > 0`, and `item_count >= 1`. Skip silently (with a log entry) if the guard fails rather than sending malformed events.
  - **Shared event_id half: implemented, then superseded by owning the browser fire directly.** `CAPI::get_event_id(\WC_Order $order)` (`class-capi.php`) is the single read path for the stored `_rzog_capi_event_id`. `output_browser_pixel_event_id()` (hooked on `woocommerce_thankyou`) still prints `window.RZOG.purchaseEventId` for any external consumer, but now also calls a private `output_browser_purchase_pixel()` that fires `fbq('track','Purchase', {value, currency}, {eventID})` itself — loading `fbevents.js` and calling `fbq('init', pixel_id)` only if `typeof window.fbq !== 'function'` (client-side detection; this plugin can't know server-side what a theme/GTM/other plugin already injected). **Gated behind a new setting, `rzog_capi_browser_pixel_enabled` (Admin_Settings, default `no`)** — must stay `no` until the literal cutover moment, since running it alongside devpsoft's still-active browser pixel produces two non-deduped Purchase events (different event IDs). Own-side dedup against page reload/revisit uses a new `_rzog_capi_browser_sent` meta flag, separate from the server-side `_rzog_capi_sent` flag. Untested in production — needs a real test order plus Meta Events Manager Test Events confirmation, and devpsoft's pixel must be switched off in the same moment this setting flips to `yes`.
    - **devpsoft-conflict cross-check: implemented as a real check, not just a doc note.** `CAPI::devpsoft_browser_pixel_active()` calls devpsoft's own exposed API, `\DPFB_Utils::setting('enable_browser', 1)` (call signature confirmed by literal evidence in `reference/devpsoft-woocommerce-events-reference.php`; returns `false` if `DPFB_Utils` doesn't exist, i.e. devpsoft isn't installed). This is wired in three places: (1) `Admin_Settings::sanitize_browser_pixel_enabled()` is now the `sanitize_callback` for `rzog_capi_browser_pixel_enabled` — if devpsoft is detected active, the save is forced back to `no` and an `add_settings_error()` is raised (now actually visible: `render()` calls `settings_errors()`, which it previously did not); (2) the settings page shows a live, uncached devpsoft-active/inactive/not-installed indicator next to the field on every page load, not just at save time; (3) `output_browser_purchase_pixel()` itself re-checks `devpsoft_browser_pixel_active()` at the moment of firing, independent of whatever the saved setting says — closes the race where devpsoft gets reactivated after the setting was correctly saved `yes`. Caveat: this can only detect devpsoft specifically via its own API; it cannot detect some other unrelated plugin/theme/GTM tag independently firing `fbq('track','Purchase',...)` — that must still be ruled out manually before cutover.
    - **Hook-priority/execution-order relative to devpsoft's own pixel injection: unverifiable from this repo, and now moot.** `reference/devpsoft-woocommerce-events-reference.php` (the only devpsoft source present here, per CLAUDE.md's reference-folder scope) contains zero occurrences of `wp_head`, `wp_footer`, `connect.facebook.net`, `fbevents`, or any `<script` tag — its actual pixel-injection hook/timing is simply not in this codebase, so whether it loads before or after `output_browser_pixel_event_id()` (on `woocommerce_thankyou`) cannot be confirmed either way. This is why the devpsoft-conflict cross-check above checks devpsoft's *setting*, not `window.fbq`'s presence at runtime — if devpsoft is detected active the browser fire refuses to run regardless of load order, so there is nothing left to race.
  - **Pre-send validity guard: implemented.** `CAPI::send_event()` (`class-capi.php`) now checks `$order->get_status()` is `processing`/`completed`, and `$order->get_total() > 0` and `$order->get_item_count() >= 1`, before attempting any send — added because `woocommerce_thankyou` fires on every order-received page view regardless of the order's current status, unlike the two status-transition hooks. The retry-queue failure logging below is still not implemented, and the locked phone-function renames (next bullet) have not been applied yet — held as a separate, deliberately deferred pass.
