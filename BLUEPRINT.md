...
- **Purchase dedup architecture**: keep the three trigger points (`woocommerce_order_status_processing`, `woocommerce_order_status_completed`, `woocommerce_thankyou`) but formalize the guard: a persistent per-order meta flag (already-sent) checked before any send, one shared `event_id` reused across browser pixel and CAPI for the same order (for Meta-side dedup), and a **pre-send validity guard** that must pass before a Purchase event is sent at all: order status is `processing` or `completed`, `total > 0`, and `item_count >= 1`. Skip silently (with a log entry) if the guard fails rather than sending malformed events.
  - **Shared event_id half: implemented.** `CAPI::get_event_id(\WC_Order $order)` (`class-capi.php`) is now the single read path for the stored `_rzog_capi_event_id`, used by both the server-side Purchase send and a new `output_browser_pixel_event_id()` hooked on `woocommerce_thankyou`, which prints `window.RZOG.purchaseEventId` so whatever fires the browser-side pixel call (this plugin doesn't own that base pixel code) can pass the same value as `eventID` and actually dedup against the CAPI event. **The browser-side consumer of that global is unverified** — it lives outside this repo (theme/GTM/other plugin) and hasn't been confirmed to read it yet.
